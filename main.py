...
def load_data():
    try:
        ...
        bypass_roads = []
        # SHP file path එක හරියටම පරීක්ෂා කිරීම
...
if traffic_data is not None:
    ...
    # 🚦 Bypass Roads Logic (Peak times හෝ prediction වැඩි නම් පෙන්වයි)
    ...
    if (ai_pred > 40 or is_peak) and bypass_roads:
        for road in bypass_roads:
            fig_map.add_trace(go.Scattermapbox(
                mode="lines+markers", # පාර වඩාත් පැහැදිලිව පෙන්වීමට markers එක් කළා
                lat=road['lats'], 
                lon=road['lons'],
                line=dict(width=5, color='#00FFFF'), 
                name="AI Bypass Active",
                hoverinfo='text',
                text="AI Recommended Bypass Route"
            ))
